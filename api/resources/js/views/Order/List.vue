<template>
  <div>
    ...
    >
      ...
      <template v-slot:item.total="{ item }">PKR {{ item.total }} </template>

      <template v-slot:item.order_status="{ item }">
        <OrderStatus :order_status="item.order_status" />
      </template>
      <template v-slot:item.actions="{ item }">
        <v-btn v-if="permissions.indexOf(143) >= 0" color="info" fab x-small dark :to="{ name: 'auth.orders.view', params: { id: item.id } }">
